...
class AppSettings(BaseSettings):
    ...
    # IMPORTANT:
    # DatabaseManager uses synchronous SQLAlchemy,
    # so SQLite must NOT use aiosqlite here.
    database_url: str = Field(
        default="sqlite:///./ease.db",
        alias="DATABASE_URL",
    )
...
class PipelineSettings:
    ...
    def load(self) -> dict[str, Any]:
        ...
        if not self.path.exists():
            raise FileNotFoundError(
                f"Pipeline config not found: {self.path}"
            )
    ...
    def _validate(self) -> None:
        ...
        if "stages" not in self._raw:
            raise ValueError(
                "Pipeline config must contain a 'stages' list."
            )
        ...
        for i, stage in enumerate(stages):
            if not isinstance(stage, dict):
                raise ValueError(
                    f"Stage at index {i} must be a mapping."
                )

            for key in ("name", "category", "plugin"):
                if key not in stage:
                    raise ValueError(
                        f"Stage at index {i} is missing required key '{key}'."
                    )
