"""Autopep8 optimizer plugin — wraps `autopep8 --in-place --recursive`."""
...
class AutopepOptimizer:
    """Formats Python code using autopep8 (pycodestyle compliance)."""
    ...
    def run(self, ctx: ExecutionContext, input_data: BaseModel) -> OptimizedCode:
        ...
        cmd = ["autopep8", "--in-place", "--recursive", "--aggressive", str(target)]
        try:
            ...
            return OptimizedCode(
                ...
                summary=f"Error running autopep8: {exc}",
            )

        files = self._collect_modified_files(ctx.workspace.original, target)
        return OptimizedCode(
            strategy_name=self.name, files=files,
            summary=f"autopep8 formatted {len(files)} file(s).",
        )

    def healthcheck(self) -> bool:
        try:
            result = subprocess.run(["autopep8", "--version"], capture_output=True, text=True, timeout=10)
